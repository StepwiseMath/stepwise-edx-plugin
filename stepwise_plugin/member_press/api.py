...
from .utils import MBJSONEncoder, log_pretrip, log_postrip, log_trace
        ...
        try:
            ...
            raise Exception(
                "MPClient.get_token() response raised the following exception for requests.post(url={url}, data={data}, headers={headers}, verify=False)".format(
                    url=url,
                    data=json.dumps(masked_dict(dict(data)), cls=MBJSONEncoder, indent=4),
                    headers=json.dumps(masked_dict(dict(self.headers())), cls=MBJSONEncoder, indent=4),
                )
            ) from e
        ...
        if not self.is_valid_member(member):
            logger.warning(
                "get_member() returned an invalid json response {response}".format(
                    response=json.dumps(masked_dict(member), cls=MBJSONEncoder, indent=4)
                )
            )
            return False
